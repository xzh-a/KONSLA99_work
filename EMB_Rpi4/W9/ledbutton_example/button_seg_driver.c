...
static ssize_t driver_read(struct file *file, char *user_buffer, size_t count, loff_t *offs) {
    ...
    to_copy = min(count, sizeof(but_u));
    to_copy = min(count, sizeof(but_d));

    but_u = gpio_get_value(5) + '0';
    but_d = gpio_get_value(6) + '0';

    if (but_u) {
        not_copied = copy_to_user(user_buffer, "U", to_copy);
    } else if (but_d) {
        not_copied = copy_to_user(user_buffer, "D", to_copy);
    } else {
        not_copied = copy_to_user(user_buffer, "", to_copy);
    }
